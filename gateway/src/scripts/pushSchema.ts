...
import { GATEWAY_SERVICE_NAME, NODE_ENV } from '../config';
import {
  DUMMY_SERVICE_URL,
  FEDERATED_SCHEMA_FILE_PATH,
...
function getPushCommand(variant: string): string {
  return `npx apollo service:push --variant=${variant} --serviceName=${GATEWAY_SERVICE_NAME} --serviceURL=${DUMMY_SERVICE_URL} --localSchemaFile=${FEDERATED_SCHEMA_FILE_PATH}`;
}
